...
*   **`batch_update` exposes the full Sheets `batchUpdate` surface.** This is intentional but broad: an attacker-controlled prompt or sheet can request any operation the API supports, including modifying protected ranges and conditional formatting. Use `ENABLED_TOOLS` (see [Tool Filtering](#-tool-filtering-reduce-context-usage)) to disable it when not needed.
